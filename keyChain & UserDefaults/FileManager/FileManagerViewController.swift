...
//        defaults.set(contents, forKey: "Content")
//    }
    
    func savedContents() {
        if let savedContents = defaults.object(forKey: "contents") as? Data {
            if let decodedContents = try? NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(savedContents) as? [Content] {
